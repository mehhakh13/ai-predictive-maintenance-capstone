...
const generateMockCostData = () => {
  ...
  for (let i = 0; i < numOrders; i++) {
    ...
    data.push({
      ...
      BuildingLabel: buildings[Math.floor(Math.random() * buildings.length)],
...
export const useCostAnalysisData = (filters = {}) => {
  ...
  const filteredData = useMemo(() => {
    ...
    if (filters.system && filters.system !== 'All') {
      filtered = filtered.filter(item => item.SystemDescription === filters.system);
    }
    if (filters.subsystem && filters.subsystem !== 'All') {
      filtered = filtered.filter(item => item.SubsystemDescription === filters.subsystem);
    }
